...
interface Props { project: Project; nextSlug: string; nextTitle: string }

export function ProjectDetail({ project: p, nextSlug, nextTitle }: Props) {
  return (
      ...
      <div className="px-5 md:px-10 py-16">
        <div className="mx-auto" style={{ maxWidth: '1100px' }}>
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10">
            ...
            <div className="space-y-4">
              {p.chartData && <ProjectChartLoader data={p.chartData} accentColor={p.accentColor} />}
              <motion.div initial="hidden" whileInView="show" variants={fadein} viewport={{ once: true }}>
                <div className="p-5 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {p.tech.map(t => <span key={t} className="font-mono text-[11px] rounded px-2.5 py-1" style={{ color: 'var(--blue)', background: 'var(--blue-bg)', border: '1px solid var(--blue-border)' }}>{t}</span>)}
                  </div>
                </div>
              </motion.div>
              {p.liveUrl && (
      ...
      <div style={{ borderTop: '1px solid var(--border)', background: 'var(--surface)' }}>
        <Link href={`/projects/${nextSlug}`}>
          <div className="mx-auto flex items-center justify-between px-5 md:px-10 py-6 group" style={{ maxWidth: '1100px' }}>
            ...
            <span className="font-display font-[700] text-[16px] group-hover:text-[color:var(--blue)] transition-colors duration-150" style={{ color: 'var(--navy)' }}>{nextTitle} →</span>
          </div>
        </Link>
      </div>
    </main>
